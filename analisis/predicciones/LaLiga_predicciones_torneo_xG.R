...
# Carga de datos
dta <- read.csv("datos/la_liga/agregado_temporadas/temporadas_18_25.csv")

temporada <- dta[dta$Fecha >= "2024-11-22", ] # Temporada a analizar
...
expectedGoals <- function(modelo, local, rival, localia) {
  return(predict(
      modelo, 
      data.frame(
...
write.csv(temporada, file = "resultados/predicciones/resultados_temporada_xG_v2.csv", row.names = FALSE)